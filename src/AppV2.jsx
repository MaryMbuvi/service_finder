    ...
    >
      ...
      <div className="w-full max-w-[1240px] bg-gradient-to-b from-[#F4FFFB] to-white rounded-[2rem] border-[6px] border-[#FCE8ED] shadow-sm overflow-hidden pt-6 pb-16">
        ...
        <div id="center" className="w-full mx-auto px-4 sm:px-8 space-y-8 flex flex-col items-center flex-grow">
          ...
          <div className="w-full flex flex-col lg:flex-row gap-6 lg:items-start items-center">
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="font-extrabold text-base sm:text-lg text-[#034B41]">Your details</h3>
                  {missingDetails ? (
                    ...
                    <p className="text-[11px] sm:text-xs mt-0.5 text-[#6C768E] font-semibold">Entering this accurately auto-filters clinics to match the healthcare laws in your state.</p>
                  )}
                </div>
                
                <div className="flex flex-col sm:flex-row gap-4 w-full">
                  <div className="w-full sm:w-1/2 flex flex-col gap-1.5">
                    <span className="text-[11px] sm:text-xs font-bold tracking-wider pl-1 text-slate-700">Select your age group:</span>
                    <div className={`grid grid-cols-2 p-1 bg-[#FDFAF9] border rounded-xl w-full ${missingDetails && !ageGroup ? 'border-red-400' : 'border-gray-200'}`}>
                      <button 
                        onClick={() => { setAgeGroup('under_18'); setMissingDetails(false); }}
                        className={`py-2.5 text-xs sm:text-sm font-black rounded-lg transition-all cursor-pointer ${ageGroup === 'under_18' ? 'bg-[#034B41] text-white shadow-sm' : 'text-[#6C768E] hover:text-[#034B41] hover:bg-slate-100'}`}
                      >
                        Under 18
                      </button>
                      <button 
                        onClick={() => { setAgeGroup('18_plus'); setMissingDetails(false); }}
                        className={`py-2.5 text-xs sm:text-sm font-black rounded-lg transition-all cursor-pointer ${ageGroup === '18_plus' ? 'bg-[#034B41] text-white shadow-sm' : 'text-[#6C768E] hover:text-[#034B41] hover:bg-slate-100'}`}
                      >
                        18 or older
                      </button>
                    </div>
                  ...
                  <div className="w-full sm:w-1/2 flex flex-col gap-1.5">
                    <span className="text-[11px] sm:text-xs font-bold tracking-wider pl-1 text-slate-700">Select your state:</span>
                    <div className="relative w-full">
                      <select 
                        value={stateLocation} 
                        onChange={(e) => { setStateLocation(e.target.value); setMissingDetails(false); }}
                        className={`w-full p-3 pl-4 pr-10 text-sm font-bold bg-[#FDFAF9] border rounded-xl text-[#034B41] appearance-none focus:outline-none focus:border-[#CEC1F0] transition-all cursor-pointer ${missingDetails && !stateLocation ? 'border-red-400' : 'border-gray-200'}`}
                      >
                ...
                <div className="flex flex-wrap gap-3 w-full">
                  {SERVICES.map((srv) => (
                    <button
                      ...
                      /* 🔧 STYLING UPDATE: Matches image_2399e6.png with clean border, custom soft slate font, and seamless d7efe7 mint background on hover */
            ...
            >
              ...
              <div className="bg-[#FBF9F8] border-r border-purple-50 p-5 md:w-5/12 space-y-5 text-xs sm:text-sm">
                {selectedService === 'abortion' && (
                  <div className="space-y-3">
                    <h3 className="text-xs font-black text-[#034B41] uppercase tracking-wider">How many weeks pregnant?</h3>
                    ...
                    {weeksPregnant === 'under10' && (
                      <div className="p-4 bg-purple-50 border border-[#E0D6FA] rounded-xl text-xs sm:text-sm text-purple-950 font-medium leading-relaxed animate-fade-in">
                        <span className="font-extrabold block text-sm mb-1 text-[#034B41]">✨ Recommended: Pills By Mail</span>
                    ...
                    {(weeksPregnant === '10to15' || weeksPregnant === 'over15') && (
                      <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-xs sm:text-sm text-amber-950 font-medium leading-relaxed animate-fade-in">
                        <span className="font-extrabold block text-sm mb-1 text-amber-800">🏥 Recommended: In-Person Care</span>
                ...
                {selectedService === 'testing' && (
                  <div className="space-y-3">
                    <h3 className="text-xs font-black text-[#034B41] uppercase tracking-wider">Do you feel active symptoms?</h3>
                    <div className="flex flex-col gap-2">
                    ...
                    {hasStiSymptoms === 'yes' && (
                      <div className="p-4 bg-purple-50 border border-[#E0D6FA] rounded-xl text-xs sm:text-sm text-purple-950 font-medium leading-relaxed animate-fade-in">
                        <span className="font-extrabold block text-sm mb-1 text-[#034B41]">🏥 Recommended: In-Person Testing</span>
                    ...
                    {hasStiSymptoms === 'no' && (
                      <div className="p-4 bg-purple-50 border border-[#E0D6FA] rounded-xl text-xs sm:text-sm text-purple-950 font-medium leading-relaxed animate-fade-in">
                        <span className="font-extrabold block text-sm mb-1 text-[#034B41]">✨ Recommended: Home Test Kits</span>
                ...
                {selectedService === 'contraceptive' && (
                  <div className="space-y-3">
                    <h3 className="text-xs font-black text-[#034B41] uppercase tracking-wider">Is this an emergency accident?</h3>
                    <div className="flex flex-col gap-2">
                    ...
                    {contraceptiveUrgency === 'emergency' && (
                      <div className="p-4 bg-purple-50 border border-[#E0D6FA] rounded-xl text-xs sm:text-sm text-purple-950 font-medium leading-relaxed animate-fade-in">
                        <span className="font-extrabold block text-sm mb-1 text-[#034B41]">⚡ Prioritizing Emergency Access</span>
                ...
                {selectedService === 'pregnancy' && (
                  <div className="p-4 bg-purple-50 border border-[#E0D6FA] rounded-xl text-xs sm:text-sm text-purple-950 font-medium leading-relaxed">
                    <span className="font-extrabold block text-sm mb-1 text-[#034B41]">🤰 Confidential Verification</span>
                ...
                {selectedService === 'lgbtq' && (
                  <div className="space-y-3">
                    <h3 className="text-xs font-black text-[#034B41] uppercase tracking-wider">What community resource do you need?</h3>
                    <div className="flex flex-col gap-2">
                      {[
                ...
                {selectedService === 'mental' && (
                  <div className="space-y-3">
                    <h3 className="text-xs font-black text-[#034B41] uppercase tracking-wider">What care mode is needed?</h3>
                    <div className="flex flex-col gap-2">
                ...
                {selectedService === 'gbv' && (
                  <div className="space-y-3">
                    <h3 className="text-xs font-black text-[#034B41] uppercase tracking-wider">What do you need right now?</h3>
                    <div className="flex flex-col gap-2">
                      {[
                ...
                <div className="pt-4 border-t border-gray-200 space-y-2">
                  <h3 className="text-xs font-black uppercase text-[#034B41] tracking-wider">
              ...
              <div className="p-5 bg-white md:w-7/12 flex flex-col relative space-y-4 text-xs sm:text-sm">
                ...
                <div className="flex items-center justify-between border-b border-purple-50 pb-3">
                  <h3 className="text-xs font-black uppercase text-[#034B41] tracking-wider">Recommended providers</h3>
                  <span className="text-[10px] sm:text-xs font-black bg-[#E0D6FA] text-[#034B41] px-3 py-1 rounded-full uppercase">{activeResources.length} Found</span>
                </div>
                    ...
                    activeResources.map((fac, idx) => (
                      <div key={idx} className="p-4 bg-white border border-gray-100 shadow-sm rounded-2xl flex flex-col justify-between gap-3 hover:border-purple-200 transition-colors">
                        ...
                        <div className="flex items-center justify-between pt-2.5 border-t border-gray-50">
                          <span className="text-[9px] sm:text-xs bg-[#FDFAF9] border border-purple-100 text-[#034B41] font-extrabold px-2 py-0.5 rounded uppercase tracking-wider">
          ...
          {/* ============================================================ */}
          {/* 🆘 CONDENSED 24/7 EMERGENCY CRISIS HELPLINES TRACK           */}
          {/* ============================================================ */}
          <div id="next-steps" className="w-full pt-8 py-6 my-4 border-t border-gray-200 flex flex-col gap-6">
                ...
                <div key={lIdx} id={lIdx === 0 ? "docs" : undefined} className="p-5 bg-white border border-purple-50 rounded-2xl flex flex-col justify-between gap-4 shadow-sm hover:border-purple-200 transition-colors">
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between gap-1.5 flex-wrap">
                      <span className="text-[9px] sm:text-xs bg-teal-50 border border-teal-100 text-[#034B41] font-extrabold px-2.5 py-1 rounded-md uppercase tracking-wider">{line.group}</span>
                    </div>
            ...
            <div className="w-full bg-[#F4FFFB] border border-teal-100 p-5 rounded-xl text-center shadow-sm max-w-3xl mx-auto mt-2 animate-fade-in text-sm sm:text-base">
              <p className="font-bold text-[#034B41] leading-relaxed">
                ...
                <a 
                  ...
                  className="text-purple-600 underline font-black hover:text-purple-800 transition-colors"
                >
                  here
                </a>.
              </p>
